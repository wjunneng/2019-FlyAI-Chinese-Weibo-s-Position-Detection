...
class Instructor(object):
    """
    特点：使用flyai字典的get all data  | 自己进行划分next batch | 使用交叉验证
    """
    ...
    def __init__(self, arguments):
        ...
        valset_len = len(trainset) // self.arguments.cross_val_fold
        self.splitedsets = random_split(trainset, tuple([valset_len] * (self.arguments.cross_val_fold - 1) + [
            len(trainset) - valset_len * (self.arguments.cross_val_fold - 1)]))
    ...
    def run(self):
        ...
        for fid in range(self.arguments.cross_val_fold):
            logger.info('fold : {}'.format(fid))
            logger.info('>' * 100)

            trainset = ConcatDataset([x for i, x in enumerate(self.splitedsets) if i != fid])
            valset = self.splitedsets[fid]

            train_data_loader = DataLoader(dataset=trainset, batch_size=self.args.BATCH, shuffle=True)
            val_data_loader = DataLoader(dataset=valset, batch_size=self.args.BATCH, shuffle=False)

...
import numpy as np
import pandas as pd
from BERT.data_utils import PreProcessing
...
def replacement_order(predict_dir, predict_order_dir, taskaa_order_dir):
    ...
    with open(predict_dir, 'r') as file_1:
        ...
        data = pd.DataFrame(data={'stance': stance, 'target': target, 'text': text})

        result = None
        for topic in topics:
            current_data = data[data['target'] == topic]
            current_data['length'] = current_data['text'].apply(lambda x: len(x))
        ...
        result.to_csv(path_or_buf=predict_order_dir, sep='\t')

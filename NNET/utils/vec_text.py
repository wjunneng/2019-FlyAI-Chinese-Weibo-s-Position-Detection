...
def make_data(raw_data_i, vocab, word2idx, embeddings, label2idx=None, big_voc=False, feat_names=None):
    """
    ...
    if big_voc:
        if label2idx is None:
            for idx in range(len(feat_names)):
                ...
                data[feat_name] = sentences_to_idx(raw_data_i[idx], word2idx)
        ...
        if label2idx is None:
            for idx in range(len(feat_names)):
                data[feat_names[idx]] = sentences_to_idx_small_vocab(raw_data_i[idx], vocab, word2idx, embeddings,
                                                                     prompt=feat_names[idx])
        else:
            feats = raw_data_i[:-1]
            labels = raw_data_i[-1]
            for idx in range(len(feat_names) - 1):
                feat_name = feat_names[idx]
                data[feat_name] = sentences_to_idx_small_vocab(feats[idx], vocab, word2idx, embeddings,
                                                               prompt=feat_name)
...
def make_datasets(vocab, raw_data, label2idx, big_voc=False, feat_names=None):
    ...
    if big_voc:
        # padding and unk available
        word2idx = vocab["word2idx"]
        embeddings = vocab["embeddings"]
    else:
        # no padding or unk available Add PADDING, UNK to word2idx and embeddings in the small setting
    ...
    if feat_names is None:
        feat_names = ["xIndexes", "xQuestions"]
        datasets['prediction'] = make_data(raw_data_i=raw_data['prediction'], vocab=vocab, word2idx=word2idx,
                                           embeddings=embeddings, label2idx=label2idx, big_voc=big_voc,
        ...
        for name in names:
            ...
            datasets[name] = make_data(raw_data_i=raw_data[name], vocab=vocab, word2idx=word2idx, embeddings=embeddings,
                                       label2idx=datasets["label2idx"], big_voc=big_voc, feat_names=feat_names)
...
def load_tvt(tvt_set, max_lens, feat_names=None):
    ...
    if not feat_names:
        ...
        tvt = YDataset(list_of_features=feats,
                       labels=None,
                       to_pad=True,
                       max_lens=max_lens)
    else:
        feats = [tvt_set[key] for key in feat_names]
        tvt = YDataset(list_of_features=feats[:-1],
                       labels=feats[-1],
                       to_pad=True,
                       max_lens=max_lens)
    return tvt
